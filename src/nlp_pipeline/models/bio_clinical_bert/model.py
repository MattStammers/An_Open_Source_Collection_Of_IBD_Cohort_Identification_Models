"""
Module: biomedicalbert model.py

Description:
    This module contains functions and classes for training, evaluation, and explainability
    of a BioClinicalBertv (IBD Bert) model for sequence classification. It includes utilities for reproducibility,
...
class TextDataset(Dataset):
    """
    Custom Dataset for bio_clinical_bert training and evaluation.
    ...
    Attributes:
        ...
        tokeniser (BertTokenizer): Tokeniser instance for encoding texts.
        max_length (int): Maximum token sequence length. Defaults to 512.
    """

    def __init__(self, texts, labels, tokenizer: BertTokenizer, max_length: int = 512):
        self.texts = texts
        self.labels = labels
...
def tokens_with_shap_as_string(tokenizer, input_ids_row, shap_row, top_k=10):
    ...
    Args:
        tokeniser (BertTokenizer): Tokeniser for decoding token IDs.
...
def run_shap_analysis(
    ...
    Args:
        model (torch.nn.Module): Trained bio_clinical_bert model.
        tokeniser (BertTokenizer): Tokeniser for encoding texts.
...
def run_integrated_gradients(
    ...
    try:
        ...
        for start_idx in range(0, len(sample_val_texts), ig_chunk_size):
            ...
            # Obtain embeddings.
            embeddings = model_for_ig.bio_clinical_bert.embeddings(input_ids)

            # Build baseline embeddings.
            pad_token_id = tokenizer.pad_token_id
            baseline_input_ids = torch.full_like(input_ids, pad_token_id)
            baseline_embeddings = model_for_ig.bio_clinical_bert.embeddings(
                baseline_input_ids
            )
...
def train_bio_clinical_bert_for_column(
    train_df,
    val_df,
    report_col: str,
    label_col: str,
    ...
    """
    Train a bio_clinical_bert model on a specific report column to predict a given label.

    The process includes:
        - Preprocessing the training and validation data.
        - Setting up datasets and dataloaders.
        - Initialising and training the bio_clinical_bert model.
    ...
    Args:
        ...
        tokeniser (BertTokenizer): Tokeniser for the model.
    ...
    Returns:
        tuple: (model, tokeniser, val_preds, formatted_metrics) where:
            - model: The trained bio_clinical_bert model.
            - tokeniser: The tokeniser used.
...
def predict_bio_clinical_bert(
    ...
    """
    Generate predictions for a given report column using the trained bio_clinical_bert model.
...
if __name__ == "__main__":
    ...
    logging.info(
        "This module contains bio_clinical_bert training + explainability functions."
    )
    logging.info(
        "Import and use train_bio_clinical_bert_for_column(...) / predict_bio_clinical_bert(...) in your pipeline."
    )
