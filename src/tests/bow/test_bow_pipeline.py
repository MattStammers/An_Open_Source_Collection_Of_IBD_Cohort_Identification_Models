...
@pytest.fixture
def dummy_df():
    """Synthetic dataset with class balance and realistic demographic bins for BOW pipeline tests."""
    return pd.DataFrame(
        {
            "text_column": [
...
def override_constants(
    ...
    monkeypatch.setattr(c, "BOW_ANALYSIS_DIR", str(analysis_root))
...
def pipeline_module(
    ...
    pkg_path = f"nlp_pipeline.models.{pipeline_name}.pipeline"
...
def test_pipeline_creates_expected_artifacts(
    ...
    # ------------------- CSVs ----------------------
    raw_analysis = analysis_root / "eval_bow_raw.csv"
    raw_results = results_root / "eval_bow_raw.csv"
    umls_analysis = analysis_root / "eval_bow_umls.csv"
