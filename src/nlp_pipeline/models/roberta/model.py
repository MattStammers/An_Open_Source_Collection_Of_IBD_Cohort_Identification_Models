"""
Module: roberta model.py

Description:
    This module contains functions and classes for training, evaluation, and explainability
    of a roberta model for sequence classification. It includes utilities for reproducibility,
...
class TextDataset(Dataset):
    """
    Custom Dataset for roberta training and evaluation using RobertaTokenizer.
    ...
    Attributes:
        ...
        tokeniser (RobertaTokenizer): Tokeniser instance for encoding texts.
        max_length (int): Maximum token sequence length. Defaults to 512.
    """

    def __init__(
        self, texts, labels, tokenizer: RobertaTokenizer, max_length: int = 512
    ):
        self.texts = texts
        self.labels = labels
...
def combine_subwords(tokens, attributions):
    """
    Combine subword tokens (those starting with '▁' or '##', etc.) with their preceding tokens,
    ...
    for token, attr in zip(tokens, attributions):
        if token.startswith("##") or token.startswith("▁") or token.startswith("Ġ"):
            ...
            # For example, remove "Ġ", "▁", or "##" and add to current_token.
            cleaned = token.lstrip("▁Ġ#")
...
def tokens_with_shap_as_string(tokenizer, input_ids_row, shap_row, top_k=10):
    ...
    # Identify special tokens (for RoBERTa, e.g. "<s>", "</s>", "<pad>")
...
def run_shap_analysis(
    ...
    Args:
        model (torch.nn.Module): Trained RoBERTa-based model.
        tokeniser (RobertaTokenizer): Tokeniser for encoding texts.
    ...
    try:
        ...
        for start_idx in range(0, num_val, shap_chunk_size):
            ...
            # If the model has 2 output classes, shap_values could be a list [class0, class1]
            if isinstance(chunk_values, list):
                # We only want the shap values for the positive class (index=1)
        ...
        for i in keep_idx:
            tok = raw_tokens[i]
            feature_names.append(tok.lstrip("▁Ġ#"))
...
def run_integrated_gradients(
    ...
    try:
        ...
        # For a RobertaForSequenceClassification, we can get embeddings from model_for_ig.roberta.embeddings
        def forward_func_embeds(embeds, attention_mask):
            # The typical naming inside RobertaForSequenceClassification:
            # model_for_ig.roberta -> the main RoBERTa backbone
            # Then classification_head or similar layers afterwards.

            roberta = model_for_ig.roberta
            classifier = model_for_ig.classifier

            outputs = roberta(inputs_embeds=embeds, attention_mask=attention_mask)
        ...
        for start_idx in range(0, len(sample_val_texts), ig_chunk_size):
            ...
            # Obtain embeddings.
            embeddings = model_for_ig.roberta.embeddings(input_ids)

            # Build baseline embeddings.
            pad_token_id = tokenizer.pad_token_id
            baseline_input_ids = torch.full_like(input_ids, pad_token_id)
            baseline_embeddings = model_for_ig.roberta.embeddings(baseline_input_ids)
...
def run_attention_visualization(
    ...
    """
    Visualise attention weights for the <s> token across layers and heads, saving the results as heatmaps.

    The process involves:
        - Tokenizing and batching validation texts.
        - Computing attention weights from the model.
        - Averaging the attention weights across layers and heads.
        - Extracting and plotting the attention from the <s> token.
    ...
    try:
        ...
        for start_idx in range(0, len(sample_val_texts), attn_chunk_size):
            ...
            with torch.no_grad():
                ...
                # outputs_attn.attentions is a tuple of shape (num_layers, batch_size, num_heads, seq_len, seq_len)
            ...
            for i, text in enumerate(sub_texts):
                ...
                # Typically, the <s> token is the first token in RoBERTa.
                # We visualize attention from index 0.
                cls_attn = avg_attn[i][0, :]
                # Filter out pads
                non_pad_indices = [j for j, t in enumerate(tokens) if t != "<pad>"]
                ...
                plt.xlabel("Tokens")
                plt.title(f'Attention of <s> => tokens for "{text[:50]}..."')
                plt.tight_layout()
...
def train_roberta_for_column(
    ...
    """
    Train a RoBERTa-based model on a specific report column to predict a given label.

    The process includes:
        - Preprocessing the training and validation data.
        - Setting up datasets and dataloaders.
        - Initialising and training the model (RobertaForSequenceClassification).
    ...
    Args:
        ...
        tokeniser (RobertaTokenizer): Tokeniser for the model.
    ...
    Returns:
        tuple: (model, tokenizer, val_preds, formatted_metrics) where:
            - model: The trained RobertaForSequenceClassification model.
    ...
    model = RobertaForSequenceClassification.from_pretrained(
        "allenai/biomed_roberta_base", num_labels=2, local_files_only=True
    )
    model.to(device)
...
def predict_roberta(
    ...
    """
    Generate predictions for a given report column using the trained RoBERTa-based model.

    Depending on the 'proba' flag, this function returns either class predictions (0/1) or
    the probability for the positive class.

    Args:
        model (torch.nn.Module): Trained RobertaForSequenceClassification model.
        tokeniser (RobertaTokenizer): Tokeniser for encoding texts.
...
if __name__ == "__main__":
    ...
    logging.info(
        "This module contains RoBERTa-based training + explainability functions."
    )
    logging.info(
        "Import and use train_roberta_for_column(...) / predict_roberta(...) in your pipeline."
    )
