...
@pytest.fixture
def dummy_df():
    """Synthetic dataset with class balance and realistic demographic bins for roberta pipeline tests."""
    return pd.DataFrame(
        {
            "text_column": [
...
def override_constants(
    ...
    monkeypatch.setattr(c, "ROBERTA_ANALYSIS_DIR", str(analysis_root))
...
def stub_pipeline_apis(monkeypatch: pytest.MonkeyPatch, pipeline_module: ModuleType):
    ...
    monkeypatch.setattr(pipeline_module, "predict_roberta", fake_predict, raising=True)
    ...
    def fake_configure_logging(log_dir: Path, custom_logger: str | None = None):
        ...
        log.info("roberta pipeline complete")
...
@patch(
    ...
    new=MagicMock(
        return_value=RobertaTokenizer.from_pretrained(
...
def test_pipeline_creates_expected_artifacts(
    ...
    # ------------------- CSVs ----------------------
    raw_analysis = analysis_root / "eval_roberta_raw.csv"
    raw_results = results_root / "eval_roberta_raw.csv"
    umls_analysis = analysis_root / "eval_roberta_umls.csv"
