...
model with BOW features for text classification. It now uses GridSearchCV
for better results.
...
def train_bow_model(
    ...
    *,
    ...
    """Fit a CountVectoriser + LogReg model.
    ...
    if text_col not in df.columns or label_col not in df.columns:
        logging.warning(
            "Missing text or label column - skipping model for '%s'", text_col
        )
        return None, 0.0

    data = df.dropna(subset=[text_col, label_col])
    if data.empty or len(data) == 0:
        logging.warning("No usable rows with both text and label for '%s'", text_col)
        return None, 0.0
    ...
    if len(X) == 0:
        logging.warning("After coercion, still no samples for '%s'", text_col)
        return None, 0.0
    ...
    logging.info("Trained BOW model for '%s' in %.2fs", text_col, elapsed)
...
def predict_bow(model: Optional[Pipeline], df, text_col: str, *, proba: bool = False):
