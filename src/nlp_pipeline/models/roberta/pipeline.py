...
roberta text-classification models under the unified evaluation
...
Setup
  ...
  B.  Train a DisilBERT classifier for every free-text column at **document**
...
from nlp_pipeline.models.roberta.model import predict_roberta, train_roberta_for_column
...
def run_roberta_workflow(
    ...
    *,
    ...
    """
    Train, evaluate and analyse roberta models for one text variant.

    Returns
    -------
    pd.DataFrame - concatenated evaluation summary for this *mode*.
    """
    logging.info("[%s] roberta workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        model, _, _, _ = train_roberta_for_column(
            train_df,
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_roberta"] = predict_roberta(
                model, tokenizer, split_df, report_col
            )
            # probability output (if supported)
            split_df[f"{report_col}_Prob_roberta"] = predict_roberta(
    ...
    # ----------------------------------------------------------------- #
    cum_pred_cols = [f"{rc}_Pred_roberta" for rc in c.COLWISE_COLUMNS]
    cum_proba_cols = [f"{rc}_Prob_roberta" for rc in c.COLWISE_COLUMNS]

    for df in (train_df, val_df):
        df["Cumulative_Pred_roberta"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_roberta"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_roberta": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_roberta",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    # ----------------------------------------------------------------- #
    # helper: aggregate Cumulative_Pred_roberta up to patient
    ...
    for split_name, split_df in (
        ...
        eval_df = evaluate(
            pat_from_doc,
            {"Cumulative_Pred_roberta": "Cumulative_Patient_Gold"},
    ...
    for report_col, label_col in c.IBD_COLUMNS.items():
        ...
        pred_col = f"Patient_{report_col}_Pred_roberta"
        prob_col = f"{pred_col}_Prob"
        patient_pred_cols.append(pred_col)

        for df in (train_df, val_df):
            df[pred_col] = predict_roberta(model, tokenizer, df, report_col)
            df[prob_col] = predict_roberta(model, tokenizer, df, report_col, proba=True)
    ...
    for grp_df, base_df in ((train_grp, train_df), (val_grp, val_df)):
        ...
        grp_df["Final_Prob_roberta"] = (
            base_df.groupby("study_id")["Cumulative_Prob_roberta"]
            .max()
...
def main(disable_umls: bool = False) -> None:
    """Run the full two-variant roberta pipeline (SBERT workflow)."""
    analysis_root = Path(c.ROBERTA_ANALYSIS_DIR)
    ...
    configure_logging(
        log_dir=str(c.ROBERTA_ANALYSIS_DIR),
    ...
    logging.info("Starting roberta pipeline")

    monitor = ResourceMonitor(interval=0.1)
    monitor.start()

    tracker = EmissionsTracker(
        project_name="roberta_Workflow",
    ...
    tokenizer = RobertaTokenizer.from_pretrained(
        "allenai/biomed_roberta_base", local_files_only=True
    )
    ...
    res_raw.to_csv(analysis_root / "eval_roberta_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_roberta_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls.to_csv(analysis_root / "eval_roberta_umls.csv", index=False)
    ...
    logging.info("roberta SBERT-style pipeline complete")
