...
``pytestmark = pytest.mark.parametrize('pipeline_name',["cnn"], indirect=True)``.
...
@pytest.fixture
def dummy_df():
    """Synthetic dataset with class balance and realistic demographic bins for sbert_med pipeline tests."""
    return pd.DataFrame(
        {
            "text_column": [
...
def override_constants(
    ...
    monkeypatch.setattr(c, "SBERT_MED_ANALYSIS_DIR", str(analysis_root))
...
def pipeline_module(
    ...
    monkeypatch.setattr(
        pipeline,
        "load_and_preprocess",
        lambda path, umls=False: dummy_df.copy(),
        raising=True,
    )
...
def stub_pipeline_apis(monkeypatch: pytest.MonkeyPatch, pipeline_module: ModuleType):
    ...
    def fake_train(
        ...
        **kwargs,
        ...
        (model_dir / "model.pkl").write_text("dummy model")
        # drop optional artefacts if caller requested them
        if kwargs.get("do_feature_importance"):
            (model_dir / "feature_importance.csv").write_text(
                "feature,importance\nx,1.0",
            )
        if kwargs.get("do_calibration"):
    ...
    # -------------------- predict_* ------------------------
    def fake_predict(model, df, col, proba=False):
        length = len(df)
        if proba:
    ...
    monkeypatch.setattr(pipeline_module, pred_fn, fake_predict, raising=True)
    ...
    class DummyTracker:
        ...
        def stop(self):
            (self.output_dir / "emissions.csv").write_text("energy_kwh\n0.123")
            return 0.123
    ...
    def fake_configure_logging(log_dir: Path, custom_logger: str | None = None):
        ...
        log.info("sbert_med pipeline complete")
...
def test_pipeline_creates_expected_artifacts(
    ...
    # ------------------- CSVs ----------------------
    raw_analysis = analysis_root / "eval_sbert_med_raw.csv"
    raw_results = results_root / "eval_sbert_med_raw.csv"
    umls_analysis = analysis_root / "eval_sbert_med_umls.csv"
