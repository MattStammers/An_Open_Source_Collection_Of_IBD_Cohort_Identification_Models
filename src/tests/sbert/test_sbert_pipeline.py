...
@pytest.fixture
def dummy_df():
    """Synthetic dataset with class balance and realistic demographic bins for sbert pipeline tests."""
    return pd.DataFrame(
        {
            "text_column": [
...
def override_constants(
    ...
    monkeypatch.setattr(c, "SBERT_ANALYSIS_DIR", str(analysis_root))
...
PIPELINE_NAME_MAP = {"sbert": "sbert_base", "word2vec": "word2vec", "bow": "bow"}
...
def stub_pipeline_apis(monkeypatch: pytest.MonkeyPatch, pipeline_module: ModuleType):
    ...
    def fake_configure_logging(log_dir: Path, custom_logger: str | None = None):
        ...
        log.info("SBERT pipeline complete")
...
def test_pipeline_creates_expected_artifacts(
    ...
    # ------------------- CSVs ----------------------
    raw_analysis = analysis_root / "eval_sbert_raw.csv"
    raw_results = results_root / "eval_sbert_raw.csv"
    umls_analysis = analysis_root / "eval_sbert_umls.csv"
