...
def run_bow_workflow(
    ...
    *,
    ...
    """Train, evaluate and analyse the BOW models for a given *mode*.
    ...
    logging.info("[%s] BOW workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_BOW"] = predict_bow(
                model, split_df, report_col
            )
            split_df[f"{report_col}_Prob_BOW"] = predict_bow(
    ...
    for df in (train_df, val_df):
        df["Cumulative_Pred_BOW"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_BOW"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_BOW": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_BOW",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    train_doc2pat["Cumulative_Pred_BOW"] = train_doc2pat[doc_pred_cols].max(axis=1)
    val_doc2pat["Cumulative_Pred_BOW"] = val_doc2pat[doc_pred_cols].max(axis=1)

    # 4) Final mapping: cumulative pred → patient gold
    final_mapping = {"Cumulative_Pred_BOW": "Patient_Has_IBD"}
...
def main(disable_umls: bool = False) -> None:
    ...
    analysis_root = Path(c.BOW_ANALYSIS_DIR)
    ...
    res_raw = run_bow_workflow(
        train_raw, val_raw, mode="raw", analysis_root=analysis_root
    )
    res_raw.to_csv(analysis_root / "eval_bow_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_bow_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls = run_bow_workflow(
            train_umls, val_umls, mode="umls", analysis_root=analysis_root
        )
        res_umls.to_csv(analysis_root / "eval_bow_umls.csv", index=False)
