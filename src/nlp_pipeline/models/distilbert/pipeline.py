...
DistilBERT text-classification models under the unified evaluation
...
Setup
  ...
  B.  Train a DistilBERT classifier for every free-text column at **document**
...
from nlp_pipeline.models.distilbert.model import (
    predict_distilbert,
    train_distilbert_for_column,
)
...
def run_distilbert_workflow(
    ...
    *,
    ...
    """
    Train, evaluate and analyse DistilBERT models for one text variant.

    Returns
    -------
    pd.DataFrame - concatenated evaluation summary for this *mode*.
    """
    logging.info("[%s] DistilBERT workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        model, _, _, _ = train_distilbert_for_column(
            train_df,
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_DistilBERT"] = predict_distilbert(
                model, tokenizer, split_df, report_col
            )
            # probability output (if supported)
            split_df[f"{report_col}_Prob_DistilBERT"] = predict_distilbert(
    ...
    # ----------------------------------------------------------------- #
    cum_pred_cols = [f"{rc}_Pred_DistilBERT" for rc in c.COLWISE_COLUMNS]
    cum_proba_cols = [f"{rc}_Prob_DistilBERT" for rc in c.COLWISE_COLUMNS]

    for df in (train_df, val_df):
        df["Cumulative_Pred_DistilBERT"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_DistilBERT"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_DistilBERT": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_DistilBERT",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    # ----------------------------------------------------------------- #
    # helper: aggregate Cumulative_Pred_DistilBERT up to patient
    def _aggregate_doc_preds_to_patient(df: pd.DataFrame) -> pd.DataFrame:
        ...
        cols = [
            "study_id",
            "Cumulative_Pred_DistilBERT",
            "Cumulative_Prob_DistilBERT",
        ] + label_cols
        sub = df[cols].copy()

        # 2) explicit fill for any missing labels or probs
        sub[label_cols] = sub[label_cols].fillna(0)
        sub["Cumulative_Prob_DistilBERT"] = sub["Cumulative_Prob_DistilBERT"].fillna(
            0.0
        )
        ...
        agg = sub.groupby("study_id", as_index=False).agg(
            {
                "Cumulative_Pred_DistilBERT": "max",
                "Cumulative_Prob_DistilBERT": "max",
    ...
    for split_name, split_df in (
        ...
        eval_df = evaluate(
            pat_from_doc,
            {"Cumulative_Pred_DistilBERT": "Cumulative_Patient_Gold"},
    ...
    for report_col, label_col in c.IBD_COLUMNS.items():
        ...
        pred_col = f"Patient_{report_col}_Pred_DistilBERT"
        prob_col = f"{pred_col}_Prob"
        patient_pred_cols.append(pred_col)

        for df in (train_df, val_df):
            df[pred_col] = predict_distilbert(model, tokenizer, df, report_col)
            df[prob_col] = predict_distilbert(
    ...
    for grp_df, base_df in ((train_grp, train_df), (val_grp, val_df)):
        ...
        grp_df["Final_Prob_DistilBERT"] = (
            base_df.groupby("study_id")["Cumulative_Prob_DistilBERT"]
            .max()
...
def main(disable_umls: bool = False) -> None:
    """Run the full two-variant DistilBERT pipeline (SBERT workflow)."""
    analysis_root = Path(c.DISTILBERT_ANALYSIS_DIR)
    ...
    configure_logging(
        log_dir=str(c.DISTILBERT_ANALYSIS_DIR),
    ...
    logging.info("Starting DistilBERT pipeline")

    monitor = ResourceMonitor(interval=0.1)
    monitor.start()

    tracker = EmissionsTracker(
        project_name="DistilBERT_Workflow",
    ...
    res_raw = run_distilbert_workflow(
        train_raw,
        val_raw,
        mode="raw",
    ...
    res_raw.to_csv(analysis_root / "eval_distilbert_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_distilbert_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls = run_distilbert_workflow(
            train_umls,
            val_umls,
            mode="umls",
            analysis_root=analysis_root,
        ...
        res_umls.to_csv(analysis_root / "eval_distilbert_umls.csv", index=False)
    ...
    logging.info("DistilBERT SBERT-style pipeline complete")
