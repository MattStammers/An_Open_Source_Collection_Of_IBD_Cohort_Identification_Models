...
Trains and uses a Logistic-Regression classifier whose inputs are Standard
...
class SbertEmbeddingTransformer(BaseEstimator, TransformerMixin):
    """Scikit-learn compatible transformer that converts text ⇒ SBERT embedding."""

    def __init__(
        self, model_name: str = "all-mpnet-base-v2", show_progress_bar: bool = False
    ):
    ...
    def transform(self, X, y=None):
        if self._model is None:
            raise ValueError("SbertEmbeddingTransformer has not been fitted.")
        # accept both numpy arrays and plain Python lists
        if hasattr(X, "tolist"):
            texts = X.tolist()
        else:
            texts = list(X)
...
def train_sbert_model(
    ...
    *,
    ...
    """Fit an SBERT-embedding + LogReg model.
    ...
    embed = SbertEmbeddingTransformer(
    ...
    if do_feature_importance:
        try:
            embed_t: SbertEmbeddingTransformer = pipeline.named_steps["embed"]
            clf_t = (
