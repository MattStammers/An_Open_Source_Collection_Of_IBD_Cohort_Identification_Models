...
Word2Vec pipeline
=================
A single entry point that trains, evaluates and analyses a suite of Word2Vec
models under the unified evaluation framework. It supports two text variants:
...
Setup
  ...
  B. Train a Word2Vec classifier for every free-text column at **document** level and
     at **patient** level.
...
from .model import predict_word2vec, train_word2vec_model
...
def run_word2vec_workflow(
    ...
    *,
    ...
    """Train, evaluate and analyse the Word2Vec models for a given *mode*.
    ...
    logging.info("[%s] Word2Vec workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_Word2Vec"] = predict_word2vec(
                model, split_df, report_col
            )
            split_df[f"{report_col}_Prob_Word2Vec"] = predict_word2vec(
    ...
    # ------------------------------------------------------------------- #
    cum_pred_cols = [f"{rc}_Pred_Word2Vec" for rc in c.COLWISE_COLUMNS]
    cum_proba_cols = [f"{rc}_Prob_Word2Vec" for rc in c.COLWISE_COLUMNS]

    for df in (train_df, val_df):
        df["Cumulative_Pred_Word2Vec"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_Word2Vec"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_Word2Vec": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_Word2Vec",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    doc_pred_cols = [f"{rc}_Pred_Word2Vec" for rc in c.COLWISE_COLUMNS]
    ...
    train_doc2pat["Cumulative_Pred_Word2Vec"] = train_doc2pat[doc_pred_cols].max(axis=1)
    val_doc2pat["Cumulative_Pred_Word2Vec"] = val_doc2pat[doc_pred_cols].max(axis=1)

    # 4) Final mapping: cumulative pred → patient gold
    final_mapping = {"Cumulative_Pred_Word2Vec": "Patient_Has_IBD"}
    ...
    train_eval = evaluate(
        ...
        final_mapping,
        dataset_name="Training_Set",
        pred_type="Doc2Patient_Word2Vec",
    ...
    for report_col, label_col in c.IBD_COLUMNS.items():
        ...
        pred_col = f"Patient_{report_col}_Pred_Word2Vec"
        patient_pred_cols.append(pred_col)
        for df in (train_df, val_df):
            df[pred_col] = predict_word2vec(model, df, report_col)
    ...
    for grp_df, base_df in ((train_grp, train_df), (val_grp, val_df)):
        ...
        grp_df["Final_Prob_Word2Vec"] = (
            base_df.groupby("study_id")["Cumulative_Prob_Word2Vec"]
            .max()
...
def main(disable_umls: bool = False) -> None:
    ...
    analysis_root = Path(c.WORD2VEC_ANALYSIS_DIR)
    results_dir = Path(c.FINAL_RESULTS_DIR)
    _ensure_dir(analysis_root)
    _ensure_dir(results_dir)

    configure_logging(
        log_dir=str(c.WORD2VEC_ANALYSIS_DIR),
        custom_logger=logger,
    )
    logging.info("Starting Word2Vec pipeline")

    monitor = ResourceMonitor(interval=0.1)
    monitor.start()

    tracker = EmissionsTracker(
        project_name="Word2Vec_Pipeline",
    ...
    res_raw = run_word2vec_workflow(
        train_raw, val_raw, mode="raw", analysis_root=analysis_root
    )
    res_raw.to_csv(analysis_root / "eval_word2vec_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_word2vec_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls = run_word2vec_workflow(
            train_umls, val_umls, mode="umls", analysis_root=analysis_root
        )
        res_umls.to_csv(analysis_root / "eval_word2vec_umls.csv", index=False)
    ...
    logging.info("Word2Vec pipeline complete")
