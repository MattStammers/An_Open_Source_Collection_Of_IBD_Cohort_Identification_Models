"""
Sentence-BERT (SBERT) pipeline
==============================
A single entry point that trains, evaluates and analyses a suite of
Sentence-BERT text-classification models under the unified evaluation
...
Setup
  ...
  B.  Train an SBERT classifier for every free-text column at **document**
...
from nlp_pipeline.models.sbert_base.model import predict_sbert, train_sbert_model
...
def add_doc_digest(df: pd.DataFrame) -> pd.DataFrame:
    """Append an MD5 digest of all free-text columns to *df* (column ``doc_digest``)."""
...
def _assert_data_splits_ok(
    ...
    *,
    ...
    Three conditions are checked; *ValueError* is raised or a warning logged
...
# ----------------------------------------------------------------------------#
# Core workflow for a single doc type ('raw' | 'umls')                        #
# ----------------------------------------------------------------------------#

def run_sbert_workflow(
    train_df: pd.DataFrame,
    val_df: pd.DataFrame,
    *,
    mode: str,
    ...
    """Train, evaluate and analyse the SBERT models for a given *mode*.
    ...
    """
    logging.info("[%s] SBERT workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        assert train_df[report_col].dtype == object, f"'{report_col}' is not text!"
        assert pd.api.types.is_numeric_dtype(
            train_df[label_col]
        ), f"'{label_col}' is not numeric!"
        model, _ = train_sbert_model(
            train_df,
            report_col,
            label_col,
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_SBERT"] = predict_sbert(
                model, split_df, report_col
            )
            split_df[f"{report_col}_Prob_SBERT"] = predict_sbert(
    ...
    # -------------------------------------------------------------------#
    cum_pred_cols = [f"{rc}_Pred_SBERT" for rc in c.COLWISE_COLUMNS]
    cum_proba_cols = [f"{rc}_Prob_SBERT" for rc in c.COLWISE_COLUMNS]

    for df in (train_df, val_df):
        df["Cumulative_Pred_SBERT"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_SBERT"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_SBERT": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_SBERT",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    doc_pred_cols = [f"{rc}_Pred_SBERT" for rc in c.COLWISE_COLUMNS]
    ...
    train_doc2pat["Cumulative_Pred_SBERT"] = train_doc2pat[doc_pred_cols].max(axis=1)
    val_doc2pat["Cumulative_Pred_SBERT"] = val_doc2pat[doc_pred_cols].max(axis=1)

    # 4) Final mapping: cumulative pred → patient gold
    final_mapping = {"Cumulative_Pred_SBERT": "Patient_Has_IBD"}
    ...
    for report_col, label_col in c.IBD_COLUMNS.items():
        ...
        pred_col = f"Patient_{report_col}_Pred_SBERT"
        patient_pred_cols.append(pred_col)
        for df in (train_df, val_df):
            df[pred_col] = predict_sbert(model, df, report_col)
    ...
    for grp_df, base_df in ((train_grp, train_df), (val_grp, val_df)):
        ...
        grp_df["Final_Prob_SBERT"] = (
            base_df.groupby("study_id")["Cumulative_Prob_SBERT"]
            .max()
    ...
    # -------------------------------------------------------------------#
    tmp = train_df.copy()
    text_cols = list(c.COLWISE_COLUMNS.keys())
    ...
    if pat_text["Patient_Text"].str.strip().eq("").all():
        logging.warning(f"[{mode}] Skipping Patient_Text model - all documents empty.")
    else:
        train_sbert_model(
            pat_text,
            "Patient_Text",
...
def main(
    disable_umls: bool = False,
) -> None:  # noqa: D401 – keeps mypy happy on script entry
    """CLI entry-point - executes the full two-variant SBERT pipeline."""
    analysis_root = Path(c.SBERT_ANALYSIS_DIR)
    ...
    configure_logging(
        log_dir=str(c.SBERT_ANALYSIS_DIR),
    ...
    logging.info("Starting SBERT pipeline")

    monitor = ResourceMonitor(interval=0.1)
    monitor.start()

    tracker = EmissionsTracker(
        project_name="SBERT_Pipeline",
    ...
    res_raw = run_sbert_workflow(
        train_raw, val_raw, mode="raw", analysis_root=analysis_root
    )
    res_raw.to_csv(analysis_root / "eval_sbert_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_sbert_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls = run_sbert_workflow(
            train_umls, val_umls, mode="umls", analysis_root=analysis_root
        )
        res_umls.to_csv(analysis_root / "eval_sbert_umls.csv", index=False)
    ...
    logging.info("SBERT pipeline complete")
