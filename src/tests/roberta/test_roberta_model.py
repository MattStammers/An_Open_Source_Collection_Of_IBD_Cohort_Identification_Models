...
import nlp_pipeline.models.roberta.model as mod
...
def patch_heavy(monkeypatch):
    """
    Monkey patch all heavy dependencies so the roberta training
    ...
    monkeypatch.setattr(mod, "RobertaTokenizer", DummyTokenizer)
    monkeypatch.setattr(mod, "RobertaForSequenceClassification", DummyModel)
...
def test_train_and_predict(sample_df, out_dir):
    ...
    2) Assert returned model, tokenizer, preds, and metrics.
    3) Check artifacts are created.
    4) Call predict_roberta for both probabilities and labels.
    """
    # Train with num_epochs=0 to skip the loop
    model, tokenizer, val_preds, metrics = train_roberta_for_column(
        sample_df,
        sample_df,
        "text_column",
        "label_column",
        mod.RobertaTokenizer(),
        out_dir,
        num_epochs=0,
        perform_shap=True,
        perform_lime=True,
