"""
Sentence-BERT_MED (SBERT) pipeline
==============================
A single entry point that trains, evaluates and analyses a suite of
Sentence-BERT_MED text-classification models under the unified evaluation
...
Setup
  ...
  B.  Train an SBERT_MED classifier for every free-text column at **document**
...
def run_sbert_workflow(
    ...
    *,
    ...
    """Train, evaluate and analyse the SBERT_MED models for a given *mode*.
    ...
    """
    logging.info("[%s] SBERT_MED workflow start", mode)
    ...
    for report_col, label_col in c.COLWISE_COLUMNS.items():
        ...
        for split_df in (train_df, val_df):
            split_df[f"{report_col}_Pred_SBERT_MED"] = predict_sbert(
                model, split_df, report_col
            )
            split_df[f"{report_col}_Prob_SBERT_MED"] = predict_sbert(
    ...
    # -------------------------------------------------------------------#
    cum_pred_cols = [f"{rc}_Pred_SBERT_MED" for rc in c.COLWISE_COLUMNS]
    cum_proba_cols = [f"{rc}_Prob_SBERT_MED" for rc in c.COLWISE_COLUMNS]

    for df in (train_df, val_df):
        df["Cumulative_Pred_SBERT_MED"] = df[cum_pred_cols].max(axis=1)
        df["Cumulative_Prob_SBERT_MED"] = df[cum_proba_cols].max(axis=1)

    cum_map = {"Cumulative_Pred_SBERT_MED": "Cumulative_Gold"}
    results += [
        evaluate(
            train_df,
            cum_map,
            dataset_name="Training_Set",
            pred_type="Cumulative_SBERT_MED",
            final=True,
            total_count=len(train_df),
        ),
        evaluate(
    ...
    doc_pred_cols = [f"{rc}_Pred_SBERT_MED" for rc in c.COLWISE_COLUMNS]
    ...
    train_doc2pat["Cumulative_Pred_SBERT_MED"] = train_doc2pat[doc_pred_cols].max(
        axis=1
    )
    val_doc2pat["Cumulative_Pred_SBERT_MED"] = val_doc2pat[doc_pred_cols].max(axis=1)

    # 4) Final mapping: cumulative pred → patient gold
    final_mapping = {"Cumulative_Pred_SBERT_MED": "Patient_Has_IBD"}
    ...
    train_eval = evaluate(
        ...
        final_mapping,
        dataset_name="Training_Set",
        pred_type="Doc2Patient_SBERT_MED",
    ...
    for report_col, label_col in c.IBD_COLUMNS.items():
        ...
        pred_col = f"Patient_{report_col}_Pred_SBERT_MED"
    ...
    for grp_df, base_df in ((train_grp, train_df), (val_grp, val_df)):
        ...
        grp_df["Final_Prob_SBERT_MED"] = (
            base_df.groupby("study_id")["Cumulative_Prob_SBERT_MED"]
            .max()
...
def main(disable_umls: bool = False) -> None:
    """CLI entry-point - executes the full two-variant SBERT_MED pipeline."""
    analysis_root = Path(c.SBERT_MED_ANALYSIS_DIR)
    ...
    configure_logging(
        log_dir=str(c.SBERT_MED_ANALYSIS_DIR),
    ...
    logging.info("Starting SBERT_MED pipeline")

    monitor = ResourceMonitor(interval=0.1)
    monitor.start()

    tracker = EmissionsTracker(
        project_name="SBERT_MED_Pipeline",
    ...
    res_raw.to_csv(analysis_root / "eval_sbert_med_raw.csv", index=False)
    res_raw.to_csv(results_dir / "eval_sbert_med_raw.csv", index=False)
    ...
    if not disable_umls:
        ...
        res_umls.to_csv(analysis_root / "eval_sbert_med_umls.csv", index=False)
    ...
    logging.info("SBERT Med pipeline complete")
