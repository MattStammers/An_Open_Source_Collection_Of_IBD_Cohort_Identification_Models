...
Tests for the distilbert model
...
import nlp_pipeline.models.distilbert.model as mod
...
def patch_heavy(monkeypatch):
    """
    Monkey patch all heavy dependencies so the DistilBERT training
    ...
    monkeypatch.setattr(mod, "DistilBertTokenizer", DummyTokenizer)
    monkeypatch.setattr(mod, "DistilBertForSequenceClassification", DummyModel)
...
def test_train_and_predict(sample_df, out_dir):
    ...
    4) Call predict_distilbert for both probabilities and labels.
    """
    # Train with num_epochs=0 to skip the loop
    model, tokenizer, val_preds, metrics = train_distilbert_for_column(
        sample_df,
        sample_df,
        "text_column",
        "label_column",
        mod.DistilBertTokenizer(),
        out_dir,
        num_epochs=0,
        perform_shap=True,
        perform_lime=True,
