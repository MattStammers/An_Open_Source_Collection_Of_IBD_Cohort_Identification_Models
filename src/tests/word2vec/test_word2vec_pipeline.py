...
@pytest.fixture
def dummy_df():
    """Synthetic dataset with class balance and realistic demographic bins for word2vec pipeline tests."""
    return pd.DataFrame(
        {
            "text_column": [
...
def override_constants(
    ...
    monkeypatch.setattr(c, "WORD2VEC_ANALYSIS_DIR", str(analysis_root))
...
def stub_pipeline_apis(monkeypatch: pytest.MonkeyPatch, pipeline_module: ModuleType):
    ...
    def fake_configure_logging(log_dir: Path, custom_logger: str | None = None):
        ...
        log.info("word2vec pipeline complete")
...
def test_pipeline_creates_expected_artifacts(
    ...
    # ------------------- CSVs ----------------------
    raw_analysis = analysis_root / "eval_word2vec_raw.csv"
    raw_results = results_root / "eval_word2vec_raw.csv"
    umls_analysis = analysis_root / "eval_word2vec_umls.csv"
